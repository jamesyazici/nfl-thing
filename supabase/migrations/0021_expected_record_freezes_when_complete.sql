-- weekly_leaderboard()'s expected_wins gets a third mode, per follow-up:
-- once EVERY game in the week is decided, it stops being the live
-- decided-vs-undecided hybrid from migration 0018 (which, by definition,
-- collapses to exactly equal the real correct count once nothing is left
-- undecided - not useful for comparison anymore) and instead reverts to
-- the pure PRE-GAME projection: every pick's picked_team_probability
-- snapshot from submission time, completely ignoring how any game
-- actually turned out. That's the number worth keeping forever once a
-- week wraps up - a fixed "here's what we projected" benchmark to hold
-- the final real Record up against, same mechanic as while games are
-- being played, just frozen at a different value once there's nothing
-- left to track live.
--
-- Only the expected_wins case branches on week-completeness; correct,
-- decided, upset_wins, all_forfeited, and the ORDER BY are unchanged.
...
as $$
  ...
  -- The pure pre-game projection - every pick's snapshot probability,
  -- with no regard at all for how (or whether) the game was decided. Only
  -- actually used once the whole week is complete (see week_complete
  -- below); harmless to compute unconditionally otherwise.
  per_user_frozen as (
    select
      pk.user_id,
      sum(case when pk.forfeited then 0 else coalesce(pk.picked_team_probability, 0.5) end) as frozen_expected_wins
    from picks_this_week pk
    group by pk.user_id
  ),
  week_complete as (
    select (select n from decided_count) > 0
       and (select n from decided_count) = (select n from total_games_count) as is_complete
  )
  select
    prof.id as user_id,
    prof.username,
    ...
    case
      ...
      when (select is_complete from week_complete) then coalesce(max(pf.frozen_expected_wins), 0)
  ...
  left join per_user_frozen pf on pf.user_id = prof.id
