-- Adds a per-user `forfeited` count to weekly_leaderboard() so the
-- leaderboard can show a small "(N games forfeited)" note next to the
-- record. The value is already computed internally (pick_totals.
-- forfeited_picks, used for all_forfeited) - this just also returns it.
--
-- Adding a column changes the return row type, so it's a DROP + CREATE
-- rather than CREATE OR REPLACE. Body is otherwise identical to
-- migration 0021.
...
as $$
  ...
  select
    ...
    coalesce(pt.forfeited_picks, 0) as forfeited,
    case
